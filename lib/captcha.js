...
function captcha (size) {
  return urandom(size + 200 + 100 * 4 + 1 + 1).then(function(rb) {
    ...
    return Promise.resolve({im:im, l:l })
  })
}
...
function generate (_size, _style) {
  var size = _size ? _size: SIZE
  var style = _style ? _style: -1
  const gif = Buffer.alloc(GIF_SIZE)
  return captcha(size).then(function(obj){
    const im = obj.im
    const l = obj.l
